...
from Data_Quality import join_dataframes
...
def _get_lice_df():
    lice_df = pd.read_csv('Data/combine_df_MEGA.csv')
    """
...
def update_get_lice_df(state):
    ...
    if selected_plant != "all":
        # get lon and lat for selected plant
        lon = fishplant[fishplant['name'] == selected_plant]['lon'].values[0]
        lat = fishplant[fishplant['name'] == selected_plant]['lat'].values[0]
        
        # get lice_df
        lice_df = join_dataframes(year, client_id, lat, lon)
        state["lice_df"] = lice_df
        _get_lice_df()
...
def _model_predction(state):
    ...
    if selected_lice in type_lice:
        ...
        mod = sm.tsa.statespace.SARIMAX(lice[selected_lice],
                                        ...
                                        order=(1, 1, 1), seasonal_order=(0, 0, 0, 50), trend='c')
